...
var baseURL, initURL, baseInitURL, rep, pCounter, cCounter, chunkDuration, t1, t2, url,reBufStartTime, bufferTimer, repLevel, lastChunkSize, throughput, 
	codecs, mimeType, bufferSize, maxSegNum, video, width, height, $mpd, ms = new MediaSource(), segNum=0, pStartT, cStartT, startupDelay;
var consumer = false;
var play = true;
var statistics = [];
var bwList = [];
...
function processRequest(e){	
	if (xhr.readyState == 4 && xhr.status == 200){
		...
		if (pCounter > 3 && consumer == false) {//After first 4 chunk downloaded consumer starts
			consumer = true;
			startConsumer();
		}
		pCounter++;
		...
		statistics[pCounter-1].t2 = t2;
		statistics[pCounter-1].throughput = throughput;
...
function startConsumer(){
	...
	}else if ( (pCounter - cCounter > 2) || (pCounter >= maxSegNum-4)) {//if there is at least 3 chuncks in buffer, they'll be consumed or if all chunks has already been downloaded, all will be consumed.  		
		if(pCounter == 4){
			cStartT = $.now();//Consumer start time.
			startupDelay = cStartT - pStartT;
			$('#startupDelay').append("<p> Start-up Delay : " + startupDelay + "</p>");
		}
		if (reBufStartTime != 0){
			$('#reBuffering').append("<p> re-buffering duration : " + ($.now() - reBufStartTime) + "</p>");
			reBufStartTime = 0;
		}
		cCounter++;
		setClearTime = window.setTimeout("startConsumer()",  chunkDuration*1000);//wait for play time duration of current chunk
		//console.log( "Producer : " + (pCounter) +" - Consumer : " + cCounter + " - Buffer : " + (pCounter - cCounter) );
	}else{
		reBufStartTime = $.now();//re-buffering starting time is recorded. 
		setClearTime = window.setTimeout("startConsumer()",  10);//after re-buffering, lets consumer to make checks to start again
	}
}
...
function nextChunk(){//calculates which chunk to be downloaded
	...
	if ( pCounter >= 3 ) {//if there is 4 chunks (including init chunk) at the beginning of Load, calculate next chucks' rep and return its URL
		//Calculate nextChunk		
		if (throughput >= bwList[repLevel+1]) {//if bw available increase quality level
			repLevel++;
		...
		}else if( (throughput < bwList[repLevel]) & (repLevel > 0) & ( (pCounter - cCounter +1) < (bufferSize/2) ) ){//if bw not available decrease quality level
			repLevel--;
			segmentNextURL = $($mpd).find('Representation[id = '+rep.eq(repLevel).attr('id')+'] > SegmentList > SegmentURL').eq(pCounter).attr('media');
			return nextURL = baseURL + segmentNextURL;
		}		
